...
class CubeParameters:
    def __init__(self):
        ...
        self.var_scale = (
            0.0  # variance scale dilution factor, placeholder, will be computed on initialization
        )
    ...
    def initialize(
        ...
        )  # normalization coefficient for distance
...
class CubeNode:
    ...
    def __init__(
        ...
        bayes_factor_threshold: float = 0.135,
        est_offset: float = 4.0,
        runlength_threshold: int = 5,
        discount: float = 1.0,
        quotient_limit: float = 30.0,
        max_hypothesis_ratio: float = 5.0,
        median_length: float = 11,
        blunder_min: float = 10.0,
        blunder_percent: float = 0.25,
        blunder_scalar: float = 3.0,
        capture_dist_scale: float = 0.05,
        var_scale: float = 0.0,
        dist_exponent: float = 2.0,
        stddev_to_conf_scale: float = 1.96,
        no_data_value: float = np.float32(np.nan),
        variance_selection: str = "cube",
        use_queue: bool = True,
    ...
    def remove_hypothesis(self, depth: float):
        ...
        hypo_idx = [
            self.hypotheses.index(h)
            for h in self.hypotheses
            if (abs(depth - h.current_depth) < self.depth_tolerance)
        ]
        if len(hypo_idx) == 0:
            self.logger.log(
                logging.WARNING,
    ...
    def nominate_hypothesis(self, depth: float):
        ...
        for hypo in self.hypotheses:
            ...
            if depth_difference < self.depth_tolerance:
                if (
                    min_depth_distance
                ):  # this is not the first hypothesis that we have found within the tolerance
    ...
    def monitor_hypothesis(self, hypo_index: int, new_depth: float, new_variance: float):
        ...
        if (hypo.cum_bayes_fac < self.bayes_factor_threshold) or (
            hypo.seq_length > self.runlength_threshold
        ):
            self.logger.log(
                logging.DEBUG,
    ...
    def update_hypothesis(self, hypo_index: int, depth: float, variance: float):
        ...
        if not monitoring_answer:
            self.logger.log(
                logging.DEBUG,
                "update_hypothesis: monitoring determined an intervention is required",
            )
            return False

        if self.variance_selection != "cube":
            hypo.variance_estimate = (
                hypo.number_of_points - 1
            ) * hypo.variance_estimate / hypo.number_of_points + (
                depth - hypo.current_depth
            ) ** 2 / hypo.number_of_points
        ...
        hypo.current_variance = (
            variance * hypo.predict_variance / (variance + hypo.predict_variance)
        )
    ...
    def choose_hypothesis(self):
        ...
        if second_highest_count and current_max_pointcount:
            hypo_ratio = max(
                0.0, self.max_hypothesis_ratio - (current_max_pointcount / second_highest_count)
            )
        self.logger.log(
            logging.DEBUG,
    ...
    def add_point_to_node(
        ...
        if self.predicted_depth:
            ...
            # blunders = beam solutions generated by the multibeam that do not correctly represent the seafloor
            blunder_limit = min(
                target_depth - self.blunder_min,
                target_depth - self.blunder_percent * abs(target_depth),
            )
            blunder_limit = min(
                blunder_limit, target_depth - self.blunder_scalar * np.sqrt(self.predicted_variance)
            )
    ...
    def _return_nominated_answer(self, value: tuple = ("depth", "uncertainty")):
        ...
        for value_id in value:
            if value_id == "depth":
                ...
                data.append(
                    0.0
                )  # having a nominated hypothesis means there is no ratio, only one hypothesis
            ...
            for value_id in value:
                if value_id == "depth":
                    ...
                    if self.variance_selection == "max":
                        data.append(
                            self.stddev_to_conf_scale
                            * np.sqrt(max(hyp.current_variance, hyp.variance_estimate))
                        )
...
class CubeGrid:
            ...
            for _ in range(self.num_columns):
                rdata.append(
                    CubeNode(
                        ...
                        est_offset=param.est_offset,
                        runlength_threshold=param.runlength_threshold,
                        discount=param.discount,
                        quotient_limit=param.quotient_limit,
                        median_length=param.median_length,
                        blunder_min=param.blunder_min,
                        blunder_percent=param.blunder_percent,
                        blunder_scalar=param.blunder_scalar,
                        capture_dist_scale=param.capture_dist_scale,
                        var_scale=param.var_scale,
                        dist_exponent=param.dist_exponent,
                        stddev_to_conf_scale=param.stddev_to_conf_scale,
                        no_data_value=param.no_data_value,
                        variance_selection=param.variance_selection,
                        use_queue=use_queue,
                        logger=self.logger,
                    )
                )
    ...
    def _validate_insert_points(
        ...
        assert (
            depth.size
            == horizontal_uncertainty.size
            == vertical_uncertainty.size
            == easting.size
            == northing.size
        )
    ...
    def insert_points(
        ...
        for i, z in enumerate(depth):
            ...
            if (
                max_x < 0
                or min_x >= (self.num_columns - 1)
                or max_y < 0
                or min_y >= (self.num_rows - 1)
            ):
                self.logger.log(
                    logging.DEBUG,
            ...
            for y in range(min_y, max_y + 1):
                for x in range(min_x, max_x + 1):
                    node_x = (
                        self.minimum_easting + (x * self.resolution_x) + (self.resolution_x / 2)
                    )
                    node_y = (
                        self.maximum_northing - (y * self.resolution_y) - (self.resolution_y / 2)
                    )
                ...
                if method in ["local", "posterior"]:
                    ...
                    if node_hcount <= 1:
                                ...
                                if 0 <= target_row < self.num_rows:
                                    for col_offset in range(-offset, offset + 1):
                                        ...
                                        if target_col < 0 or target_col >= self.num_columns:
                                            continue
                                        chk_node_hcount = self.grid[target_row][
                                            target_col
                                        ].return_number_of_hypotheses()
                        ...
                        if (
                            ...
                            if method == "local":
                                node_data = node.extract_closest_node_value(
                                    closest_data[0], closest_data[1], value
                                )
                            else:
                                node_data = node.extract_posterior_weighted_node_value(
                                    closest_data[0], closest_data[1], value
                                )
                elif method == "prior":
                    node_data = node.extract_node_value(value)
                    ...
                    node_data = node.extract_closest_node_value(
                        node.predicted_depth, node.predicted_variance, value
                    )
...
def run_cube_gridding(
    ...
    **kwargs,
    ...
    if method in ["local", "posterior", "prior", "predicted"]:
        cg = CubeGrid(
            minimum_easting=minimum_easting,
            maximum_northing=maximum_northing,
            num_rows=num_rows,
            num_columns=num_columns,
            resolution_x=grid_resolution_x,
            resolution_y=grid_resolution_y,
            param=cp,
            use_queue=True,
            debug=False,
        )
        cg.insert_points(depth, horizontal_uncertainty, vertical_uncertainty, easting, northing)
        depth_grid, uncertainty_grid, ratio_grid, numhyp_grid = cg.get_grid_values(
            ("depth", "uncertainty", "ratio", "n_hypotheses"), method=method
        )
    else:
