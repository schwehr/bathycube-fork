...
class CubeParameters:
    ...
    def __init__(
        ...
        self.depth_tolerance = (
            0.01  # the maximum difference allowed when searching hypotheses by depth
        )
        ...
        self.median_length = (
            11  # Length of median pre-filter sort queue (must be odd number for algorithm)
        )
        ...
        self.min_context = (
            min_context  # Minimum context search range for hypothesis disambiguation algorithm
        )
...
class Hypothesis:
    def __init__(self, initial_mean_estimate, initial_variance_estimate):
        ...
        self.predict_variance = (
            initial_variance_estimate  # current depth next-state variance prediction
        )
...
class HypothesisList:
    ...
    def get_nearest_min_error(self, depth: float, variance: float):
        ...
        idx = 0
        cur_min_error = abs(
            (depth - cur.data.predict_depth) / np.sqrt(cur.data.predict_variance + variance)
        )
        while cur.next_data is not None:
            idx += 1
            cur = cur.next_data
...
def return_new_cubegrid(
    ...
    cg = CubeGrid(
        minimum_easting, maximum_northing, num_columns, num_rows, resolution_x, resolution_y, params
    )
    return cg
...
def cube_node_remove_hypothesis(node: CubeNode, depth: numbaf32):
    ...
    if nearest_index < 0:
        if Debug:
            print(
                "cube_node_remove_hypothesis: unable to find hypothesis nearest in depth within depth tolerance"
            )
        return False
    ...
    if (
        ...
        if Debug:
            print(
                "cube_node_remove_hypothesis: removed a nominated hypothesis, clearing nominated attribute"
            )
    return True
...
def cube_node_nominate_hypothesis(node: CubeNode, depth: numbaf32):
    ...
    if node.hypotheses is None:
        if Debug:
            print(
                "cube_node_nominate_hypothesis: unable to nominate hypothesis, no hypotheses found"
            )
        return False
    node.nominated = None
    nearest_index = node.hypotheses.get_nearest_in_depth(depth, node.depth_tolerance)
    if nearest_index < 0:
        if Debug:
            print(
                "cube_node_nominate_hypothesis: unable to find hypothesis nearest in depth within depth tolerance"
            )
        return False
...
def cube_node_monitor_hypothesis(node: CubeNode, hypo_index: numbai32, new_sounding: Sounding):
    ...
    if (hypo.cum_bayes_fac < node.bayes_factor_threshold) or (
        hypo.seq_length > node.runlength_threshold
    ):
        if Debug:
            print(
...
def cube_node_update_hypothesis(node: CubeNode, hypo_index: numbai32, new_sounding: Sounding):
    ...
    hypo.current_variance = (
        new_sounding.variance
        * hypo.predict_variance
        / (new_sounding.variance + hypo.predict_variance)
    )
...
def cube_node_best_hypothesis_index(node: CubeNode, new_sounding: Sounding):
    ...
    if node.hypotheses is None:
        if Debug:
            print(
                "cube_node_best_hypothesis_index: unable to query for best hypothesis index, no hypotheses found"
            )
        return -1
...
def cube_node_update_node(node: CubeNode, new_sounding: Sounding):
    ...
    if best_idx == -1:
        ...
        if not updated:
            ...
            if Debug:
                print(
                    "cube_node_update_node: failed update, required intervention so we start a new hypothesis"
                )
        else:
            if Debug:
...
def cube_node_truncate(node: CubeNode):
    ...
    for cnt, nsounding in enumerate(queue_data):
        ...
        if quot >= node.quotient_limit:
            if Debug:
                print(
                    "cube_node_truncate: point flagged for removal with quotient value greater than limit"
                )
...
def cube_node_choose_hypothesis(node: CubeNode):
    ...
    if node.hypotheses is None:
        if Debug:
            print(
                "cube_node_choose_hypothesis: unable to query for best hypothesis index, no hypotheses found"
            )
        return None, None
    hypo_ratio = 0.0
    ...
    if current_max_pointcount != 0 and second_highest_count != 0:
        hypo_ratio = max(
            0.0, node.max_hypothesis_ratio - (current_max_pointcount / second_highest_count)
        )
...
def cube_node_queue_fill(node: CubeNode, new_sounding: Sounding):
    ...
    if node.n_queued == 0:
        ...
        if insertion_index is not None:
            ...
            if Debug:
                print(
                    "cube_node_queue_fill: adding to beginning of queue, adding depth, variance to queue"
                )
            node.queue = node.queue.prepend(new_sounding)
    node.n_queued = numbai64(node.n_queued + 1)
...
def cube_node_queue_insert(node: CubeNode, new_sounding: Sounding):
    ...
    if insertion_index is not None:
        if Debug:
            print(
                "cube_node_queue_insert: queue full, inserted at index, adding depth, variance to queue"
            )
        if insertion_index == 0:
            node.queue = node.queue.prepend(new_sounding)
        else:
        ...
        if Debug:
            print(
                "cube_node_queue_insert: queue full, adding to end of queue, adding depth, variance to queue"
            )
...
def cube_node_insert(node: CubeNode, new_sounding: Sounding, distance_to_node: numbaf32):
    ...
    if node.predicted_depth:
        ...
        # blunders = beam solutions generated by the multibeam that do not correctly represent the seafloor
        blunder_limit = min(
            target_depth - node.blunder_min, target_depth - node.blunder_percent * abs(target_depth)
        )
        blunder_limit = min(
            blunder_limit, target_depth - node.blunder_scalar * np.sqrt(node.predicted_variance)
        )
        if new_sounding.depth < blunder_limit:
            if Debug:
                print(
                    "cube_node_insert: Sounding rejected with predicted depth used, less than blunder limit"
                )
            return True
    else:
        if Debug:
    ...
    if dist > max(calculated_captdist, 0.5):
        if Debug:
            print(
                "cube_node_insert: sounding rejected, dist greater than max(0.5 or calculated capture distance)"
            )
        return True
    if Debug:
    ...
    new_sounding.variance = new_sounding.vert_unc * numbaf32(
        1.0 + node.var_scale * (dist**node.dist_exponent)
    )
...
def cube_node_get_nominated_depth_uncertainty(node: CubeNode):
    ...
    if Debug:
        print(
            "cube_node_get_nominated_depth_uncertainty: unable to get hypothesis answer, no samples found"
        )
...
def cube_node_extract_depth_uncertainty(node: CubeNode):
    ...
    if node.hypotheses.next_data is None:  # only one hypothesis
        ...
        if Debug:
            print(
                "cube_node_extract_depth_uncertainty: getting the hypothesis answer, as only one was found"
            )
        return depth, uncertainty, ratio
    ...
    if best_hypo is None:
        if Debug:
            print(
                "cube_node_extract_depth_uncertainty: no answer, failed to gather best hypothesis"
            )
        return node.no_data_value, node.no_data_value, node.no_data_value
...
def cube_node_extract_closest_depth_uncertainty(
    ...
    if (
        (node.nominated is not None)
        or (node.hypotheses is None)
        or (node.hypotheses.next_data is None)
    ):
        if Debug:
            print(
    ...
    for hyp in node.hypotheses.get_data():
        if (
            ...
            error = abs(
                (hyp.current_depth - numbaf32(query_depth)) / numbaf32(np.sqrt(query_variance))
            )
    ...
    depth, uncertainty, ratio = cube_node_get_generic_hypothesis_depth_uncertainty(
        node, nearest_hypo
    )
    ratio = nearest_ratio
    return depth, uncertainty, ratio
...
def cube_node_extract_posterior_depth_uncertainty(
    ...
    for hyp in node.hypotheses.get_data():
        if (
            ...
            posterior = -numbaf32((guide_depth - mean) ** 2) / numbaf32(
                2.0 * guide_variance
            ) + numbaf32(np.log(hyp.number_of_samples))
...
def cube_grid_insert_points(
    ...
    for i in range(depth.shape[0]):
        max_variance_allowed = (cg.iho_fixed + cg.iho_percent * depth[i] ** 2) / (
            conf_95_percent**2
        )
        ...
        for y in range(min_y, max_y + 1):
            for x in range(min_x, max_x + 1):
                ...
                snding = return_new_sounding(
                    depth[i], 0.0, vertical_uncertainty[i], horizontal_uncertainty[i]
                )
            ...
            if method in ("local", "posterior"):
                if hypcnt <= 1:
                            ...
                            if 0 <= target_row < cg.num_rows:
                                for col_offset in range(-offset, offset + 1):
                                    ...
                                    if target_col < 0 or target_col >= cg.num_columns:
                                        continue
                                    chk_node_hcount = cube_node_hypothesis_count(
                                        cg.grid[target_row][target_col]
                                    )
                                    if chk_node_hcount == 1:
                                        closest_node = cg.grid[target_row][target_col]
                                        if Debug:
                                            print(
                                                "cube_grid_extract_local: found closest node during row search"
                                            )
                                        break
                    ...
                    if (
                        ...
                        depth, uncertainty, ratio = cube_node_extract_depth_uncertainty(
                            closest_node
                        )
                        if not np.isnan(depth) and not np.isnan(uncertainty):
                            if method == "local":
                                depth, uncertainty, ratio = (
                                    cube_node_extract_closest_depth_uncertainty(
                                        node, depth, uncertainty
                                    )
                                )
                            elif method == "posterior":
                                depth, uncertainty, ratio = (
                                    cube_node_extract_posterior_depth_uncertainty(
                                        node, depth, uncertainty
                                    )
                                )
            elif method == "prior":
...
def cube_grid_numba(
    ...
    cube_grid_insert_points(
        cg, depth, horizontal_uncertainty, vertical_uncertainty, easting, northing
    )
